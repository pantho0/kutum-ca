"use client";
import MenuItemCard, {
  MenuItemCardProps,
} from "@/components/adminDashboard/MenuItemCard";
import { useGetDeletedMenu } from "@/hooks/menu.hook";
import React from "react";

function DeletedItemsPage() {
  const { data: deletedMenu } = useGetDeletedMenu();
  const deletedMenuData = deletedMenu?.data?.data;
  console.log(deletedMenuData);
  return (
    <div className="text-black font-sans px-4 sm:px-6 lg:px-8 py-6 max-w-7xl mx-auto">
      <div className="border-l-8 border-primary pl-2 sm:pl-4">
        <h2 className="text-black font-bold text-2xl sm:text-3xl md:text-4xl mb-6 sm:mb-8">
          Deleted Items
        </h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {deletedMenuData?.map((item: MenuItemCardProps, i: number) => (
          <MenuItemCard key={i} {...item} />
        ))}
      </div>
    </div>
  );
}

export default DeletedItemsPage;
